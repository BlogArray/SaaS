...
}).AddOpenIdConnect(options =>
{
    ...
    // Align the local session cookie's lifetime with the received tokens: the cookie
    // expires in absolute terms when the identity token expires instead of sliding
    // forever, so a local session can never outlive the identity provider session.
